...
#ifdef _BUILD_LIB
// ------- Code specifically designed for calls from externals languages (eg. Julia):

#include "fitness_lib.h"

/**
 * This is a global problem storing the observations for fitness calculations
 */
t_case* lib_global_problem = NULL;

/**
 * Initialization of the identification problem
 * We start with a given CA and generate imge_count sample images
 * prob - probability of introducing spatial partiality
 */
void lib_init(_LUT_ENTRY lut[], int radius, int img_count, double prob) {

    if (lib_global_problem == NULL) {
        tool_init_random();
        printf("Problem initialized, with %d images generated by the rule with radius %d given by LUT:\n", img_count, radius);
        _PRINT_ARRAY(lut, 1<<(2*radius+1));       
        t_ca *ca = ca_new_lut(lut, radius);
        t_generator_setup setup = {.mode = RANDOM, .type = ALL, .noise = 0.0};
        lib_global_problem = test_prepare_ca(ca, img_count, 1, prob, &setup);
        problem_count_cells(lib_global_problem);
        ca_free(ca);

        printf("All cells in observations:\t%d\n", lib_global_problem->masked_cells);
        printf("Partial cells in observations:\t%d (probability given: %f)\n", lib_global_problem->partial_cells, prob);
    }
    else {
        printf("Problem already initialized!\n");
    }
}

t_ca* ca_new_lut(_LUT_ENTRY lut[], int radius) {
    t_ca* ca = ca_new(radius);
    memcpy(ca->lut, lut, sizeof(_LUT_ENTRY)*ca_get_lut_len(ca));
    return ca;
}


/**
 * Calculates the fitness of the given CA defined by the LUT and radius given in the arguments
 * Could be optimized be removing the allocation at every call... but probably it wouldn't given much speedup
 */
double lib_find_fitness(_LUT_ENTRY lut[], int radius) {
    t_ca* ca = ca_new_lut(lut, radius);
    double result = evolve_fitness(ca, lib_global_problem, false);
    ca_free(ca);
    return result;
}

/**
 * For testing the ccall function
 */
void lib_echo() {
    printf("Echo\n");
}
